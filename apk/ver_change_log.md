...
### 功能
- 首次使用长期正式证书签名，为后续正式版本建立稳定的应用身份。
- 首次启动支持在图库授权前恢复旧版迁移备份。
- 恢复图片索引、AI 分析、用户修正、供应方和应用配置，但不恢复 API Key。
- 缺少凭据的供应方保持禁用，并集中提示重新填写。

### 分发
- 首次发布为 GitHub prerelease，仅供从 `v0.18.3` 执行跨签名迁移验证。
- 正式安装包不写入仓库历史 APK 目录，仅通过 GitHub Release 分发。
